export function parseTimingToSeconds(str) {
    if (!str) return null;
    let secs = 0;
...
export function formatFraction(val) {
...
export function parseServingsNum(str) {
    if (!str) return null;
    const m = String(str).match(/\d+/);
    return m ? parseInt(m[0]) : null;
}

export function splitInstruction(instruction) {
